# file: rate_laws_ca_cb_cc.py
"""
Constant-volume batch reactor solver for rate laws that depend on CA, CB, CC.

Solve CA(t), compute CB and CC from stoichiometry.

Rate law (base form):
    rate = k * (CA + add_A)**pow_A * (CB + add_B)**pow_B * (CC + add_C)**pow_C * Π(extra_constants)
...
def concs_from_CA(CA: float, CA0: float, CB0: float, CC0: float, s: StoichABC) -> Tuple[float, float]:
...
def rate_CACBCC(CA: float, CA0: float, CB0: float, CC0: float, s: StoichABC, p: CACBCCParams) -> float:
    if CA < 0:
        raise ValueError("CA must be >= 0.")
    CB, CC = concs_from_CA(CA, CA0, CB0, CC0, s)
...
def dCA_dt(t: float, CA: float, CA0: float, CB0: float, CC0: float, s: StoichABC, p: CACBCCParams) -> float:
    _ = t
    return -rate_CACBCC(CA, CA0, CB0, CC0, s, p)
...
if __name__ == "__main__":
    # Example: A + B + C -> products, rate = k*CA*CB*CC
    s = StoichABC(nuA=-1.0, nuB=-1.0, nuC=-1.0)
    p = CACBCCParams(k=0.25, pow_A=1.0, pow_B=1.0, pow_C=1.0, extra_constants={"K1": 2.0})
    t_hit = time_to_CA_target(CA0=1.0, CB0=1.5, CC0=2.0, stoich=s, params=p, CA_target=0.3, dt=1e-4, t_max=1e5)
    print("t to reach CA=0.3:", t_hit)

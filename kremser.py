"""Kremser equations for absorbers and strippers - Eqs. 22.1-24 to 22.1-29"""
...
class KremserSolver:
    """Solver using Kremser equations - Eqs. 22.1-24 to 22.1-29"""
    ...
    def _kremser_absorption(self, N: int, A: float) -> float:
        """Fraction absorbed for given N and A - Eq. 22.1-27"""
        if abs(A - 1.0) < 1e-12:
            return N / (N + 1)
        return (A ** (N + 1) - A) / (A ** (N + 1) - 1)
    
    def _kremser_stripping(self, N: int, S: float) -> float:
        """Fraction stripped for given N and S - Eq. 22.1-24"""
        if abs(S - 1.0) < 1e-12:
            return N / (N + 1)
    ...
    def solve_for_N(self, fraction: float) -> int:
        ...
        if fraction <= 0 or fraction >= 1:
            raise ChemEngError(f"Fraction must be between 0 and 1")
    ...
    def solve(self) -> Dict[str, Any]:
        ...
        if self.spec.N is not None:
            ...
            result = {"method": "given_N", "N": self.spec.N, "fraction": fraction}
        else:
            N = self.solve_for_N(self.spec.fraction)
            result = {"method": "given_fraction", "N": N, "fraction": self.spec.fraction}
...
def kremser_absorption(A: float, N: int) -> float:
    """Quick Kremser absorption calculation - Eq. 22.1-27"""
...
def kremser_stripping(S: float, N: int) -> float:
    """Quick Kremser stripping calculation - Eq. 22.1-24"""
...
def stages_for_absorption(A: float, fraction: float) -> int:
    """Number of stages needed for given absorption factor and fraction - Eq. 22.1-28"""
...
def stages_for_stripping(S: float, fraction: float) -> int:
    """Number of stages needed for given stripping factor and fraction - Eq. 22.1-25"""
